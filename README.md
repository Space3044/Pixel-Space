...
5. `/api/admin/upload` 校验表单，把压缩图写入 R2，把元数据写入 D1；如果图片公开且位置公开，会按 `location_region` 预生成静态图并写入 R2。
...
- 公开静态位置预览走 `/api/staticmap?lat=<lat>&lng=<lng>&region=<china|global>`，管理员预览走 `/api/admin/staticmap?lat=<lat>&lng=<lng>&region=<china|global>`。`region` 来自上传或编辑时手动选择的国内/国外按钮，不用经纬度自动猜。上传和管理端更新公开图公开位置时预生成静态图；访客先经 D1 可见性校验再读 R2 缓存，管理员缓存未命中时才请求上游并回写 R2。国内上游是高德 Web 服务静态地图，海外上游是 Mapbox Static Images。

...
- 地图足迹：国内足迹用高德地图，海外足迹用 Mapbox / MapLibre，位置详情静态图按手动区域分流，国内走高德 Web 服务，海外走 Mapbox Static Images，并缓存到 R2。访客只看公开且位置公开的足迹，管理员可在同一页面看到所有有坐标和区域的图片点位。
...
上传和编辑图片时，位置区域由用户在国内 / 海外搜索范围里选择，并保存为 `location_region`。这个字段决定三件事：足迹页把点位分到国内或海外地图；静态图选择高德或 Mapbox；公开接口校验缓存时必须匹配同一个区域。系统不会根据经纬度自动猜国内或海外。

国内位置搜索和地图选点走高德 JS API。海外位置搜索走管理员接口 `/api/admin/geocode`，服务端优先请求 MapTiler；MapTiler 没有结果或不可用时，再回退到 Nominatim 和 Photon。
